...
## 3.5 对照判决与处置（2026-08-26 晚，用户确认）

判决（SOFTB_verdict.md）：主门柱 FAIL（B 28.4% vs A 33.8%，-5.4pt）——但降幅在
单 run 方差带内（前科 37% 边数波动），且 novel 边本身只拉低 1.3pt；三个次级
信号正向（召回 +16 边 / novel 边内层口径 62.5% / divergence 2-3 篇 vs 前科 38）。

处置（用户确认，2026-08-27 session）：
- 改动 ②（kernel 豁免）③（归纳通道）保留——无害且次级信号正向
- 改动 ① **降级为 env 开关 `SOFT_ROUTING`**（默认 1=soft；0=恢复旧硬闸拒杀），
  多 seed 时两态都跑——从既成事实变成被测变量
- 判决推迟到多 seed（预注册本来要求正式数字多 seed，单 run 判 5.4pt 不科学）
