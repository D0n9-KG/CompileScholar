...
| R4b | 同池 rank=llm 对照（独立判分） | 0.100 | **非配对，被判分方差污染**（q2/q9 两臂判分集不同） |

R4b 与 R4 的差（-0.014）全部来自判分方差（q2 llm 臂判分器多抓 2 gold、q9 同），
非排序效应。authority 在判分相同的 q0/q8 上稳定 +0.08/+0.077。**结论：未配对
A/B 不可引用；已加 grade archive（_grade_archive.jsonl）做配对离线 A/B。**

## 召回天花板（当前池）

gold in (S2+crossref) 池 = **63/129 = 49%** → 完美判分下 recall-bound F1 ≈ 0.49。
当前 0.086-0.10 的差距构成：~51% 覆盖缺失（引文滚雪球欠账）+ 判分精度（niche
gold 全盲）+ 15 篇输出截断（q1 gold=29）。
