...
本节给出系统总体架构，论证五个核心技术设计，说明结构化输出的呈现方式，并汇总各基准上的推理效果指标与诚实边界。
...
查询理解层的核心机制是以冻结的自演化 schema 解析查询意图（机制①③）。系统先将查询映射到 schema 的模式与槽位，识别其中的领域术语，再派生两类查询变体：实体词汇查询（用超图跨论文合并学到的概念词替换泛化表述）与代表性收敛变体（向领域综述式表述收敛）。该机制的价值在独立验证集上得到量化：以召回池答案覆盖率为指标，机制使开发集提升 30.4 个百分点、留出集提升 16.7 个百分点，判据预注册、留出复核以防过拟合。需要说明的是，机制依赖查询域与图域的重叠：一项诚实的负结果是，DRL 域图对 LLM 域查询零增益，说明概念变体扩展的价值受域重叠门控，随增量建图规模增长而扩大。

\subsubsection{多源召回与语义重排}

诊断实验发现了现有检索管线的一个关键损耗源：Semantic Scholar 的 bulk 检索端点返回无相关性排序的候选流。最极端的实测案例中，被引超过三万次的领域经典论文处于匹配集内，却因随机采样从未进入返回页，任何召回策略都会随机丢失关键论文。针对该问题，系统在召回与判分之间插入语义重排层：对 2000 至 4000 篇候选池全量计算查询与标题摘要的语义相似度，以语义序选取判分窗口，使此类问题被系统性消除。注入实验的验证结果为：将 5 篇目标论文混入 85 篇混合候选池，重排后 3 篇进入前十（第 3、7、10 位），其中包含此前所有策略都无法召回的论文。该设计已在全部后续实验中启用。

\subsubsection{分块判分与混合排序}

综合排序层的判分环节存在两个工程—科学交叉问题，均通过对照实验定位并解决。其一为注意力稀释：150 篇候选整列提交判分时，判分器系统性遗漏窗口内的目标论文，实测 3/3 漏判；改为 50 篇分块后全部命中。其二为泛标题盲视：标准答案中大量论文采用泛化标题，纯标题判分实测 7/12 的在池答案被误判为不相关；将摘要引入判分输入后该盲区显著缓解。

输出排序的信号选择经三臂消融实验（纯语义、混合、纯权威）验证存在查询类型依赖性，结果如表~\ref{tab:arm} 所示。在 10 查询诊断集上，纯语义排序使气候类具体查询 $F_1$ 提升 200\%，但综述类查询下降 67\%；两类信号命门互补，最终采用语义为主、权威为辅的混合分——这正是“权衡权威性、相关性”的赛题要求在实践中的定量答案。50 查询全量的三臂对照在 10 题检查点显示三臂命中数完全持平（各 10 篇），端到端排序差异在该基准上不显著，按预注册止损规则中止并如实记录。

\begin{table}[htbp]
\centering
\small
\caption{排序信号三臂消融（10 查询诊断集，$F_1$）}
\label{tab:arm}
\begin{tabularx}{\textwidth}{l X X X}
\toprule
查询类型 & 纯权威序 & 纯语义序 & 混合（定版） \tabularnewline
\midrule
气候类具体查询 & 0.091 & 0.273（+200\%） & 兼顾 \tabularnewline
综述类宽泛查询 & 0.136 & 0.045（−67\%） & 兼顾 \tabularnewline
\bottomrule
\end{tabularx}
\end{table}
...
引用意图抽取管线从论文全文自动识别引用提及（编号式与作者—年份式双格式）并判定引用意图，将每条引文关系分类为 extends、improves、compares、replaces、adapts、background 六类。语料内引用图的构建与独立验证实现 100\% 对齐（16/16 边，意图一致 15/16）。基于该结构，机制②实现了选择性引文导航：粗召回得到的种子论文在检索时轻量入图（仅引用意图、跳过演化与对齐），并挖掘其分类引用回池。机制链的有效性得到验证：以一篇综述为种子，3.5 分钟内挖出 30 个候选。一项诚实的边界是，50 查询全量终跑在 10 题检查点与基线命中持平后按止损规则中止——覆盖增益真实存在，但漏斗逐层对冲，端到端增益未在该开发基准上确立。
...
系统在检索循环中对判分为高相关的论文自动增量建图，单篇 5 至 16 分钟、成本 0.05 至 0.2 元。构建过程包含分节抽取、跨论文概念对齐合并、引用意图标注三个阶段，全部通过事务性内核提交，具备原子性、可重放与完整审计日志。知识图以命名图形式持久存储、跨查询会话累积。自演化方面，抽取模式库随语料增量演化：离线演化出的 29 模式库相比 12 模式种子，在相同语料与判分条件下边召回提升 24\%（808 条增至 1001 条），且跨域迁移无衰退，验证了“离线演化、在线冻结”架构的增益。当前已建成 50 篇 LLM 领域论文的领域图：3827 个概念节点、3912 条超边（概念结构边 3750 条、引用意图边 162 条），模式层 218 个；检索过程的探索图另累积 122 篇种子论文并随使用持续生长。
...
在 SPARBench\cite{sparbench}（50 查询、平均 11.1 篇标准答案/查询）上，证据分两条线如实呈现。诊断线（10 查询诊断集）的迭代改进链如表~\ref{tab:diag} 所示，set-$F_1$ 从 0.007 提升至 0.093，用于逐层定位覆盖、排序、判分三处病灶，不用于声明泛化性能。泛化线（50 查询全量）的基线 micro-$F_1$ 为 0.035。
...
在官方推荐基准 AutoScholarQuery 上，gold 形态与 SPARBench 相反：平均 2.4 篇/查询，为源论文 Related Work 的引用文献，考察的是精找具体论文，赢法是引文遍历。第一阶段为失效模式诊断，定位出三类失效模式并当夜落地修复，如表~\ref{tab:fail} 所示。修复配置在留出集（q11--20，判据预注册）上的结果如表~\ref{tab:holdout} 所示：micro-$F_1$@5 从 0.000 提升至 0.150，超过预注册判定线（0.02）7 倍，为同基准已发表 S2+LLM 基线（0.004）的 34 倍，逼近裸 Google 基线（0.202）与 PaSa（0.245）。命中归因核查表明，6 个命中中 3 个由引文挖掘直接送入，3 个经术语消歧与精找判分上位，三个修复各有真实贡献。
...
在官方族基准 RealScholarQuery（PaSa 论文的真实查询基准，50 查询、平均 15.8 篇标准答案/查询）上，因提供商限流而中止的首轮运行完成 3 个查询，结果与同口径对照如表~\ref{tab:rsq} 所示。无 Google、无 API key、单免费源加超图的形态下，本系统 recall@20 达到最强 Google 基线的 92\%、超过 Google Scholar；与 PaSa-7b 的差距源于资源形态。样本量 $n=3$ 如实标注，不外推到全基准。

\begin{table}[htbp]
\centering
\small
\caption{RealScholarQuery 首测与同口径对照（micro，$n=3$）}
\label{tab:rsq}
...
\subsubsection{局限性与诚实边界}

本系统的定位建立在如实的约束与负结果之上。资源层面，Semantic Scholar 的 API key 申请未获批，当前仅匿名 bulk 端点可用，系统的 arXiv 覆盖建立在该端点之上，此约束已如实反映在检索层设计；这也是本系统与依赖 Google 爬取的系统之间存在差距的根源。机制层面，图信号若经大模型中间层会被稀释为参数先验（已实测），故图信号一律走直连路径；图内槽位直捞当前贡献为零，因离线域图尚未覆盖基准的标准答案，其价值随增量建图规模增长而扩大。评测层面，RealScholarQuery 仅完成 3 题，50 查询全量终跑按预注册止损规则中止，相关负结果均已归档。这些边界共同构成对系统能力的诚实刻画，也指明了后续迭代的方向。
