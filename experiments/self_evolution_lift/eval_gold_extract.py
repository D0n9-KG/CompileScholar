...
{{
  ...
  "law_constraints": [
    {{"law_text": "<equation/law as text>", "constrains": "<id>", "consumer": "<id>",
      "evidence": "<verbatim fragment>"}}
  ]
}}

STRICT RULES (read carefully):
1. ONLY explicit relations stated in THIS passage; each edge needs VERBATIM evidence copied from the passage.
2. Methods are named scientific entities (not generic nouns like "stress"); give aliases liberally (symbols, abbreviations, alt names).
3. EVERY edge MUST have both endpoints as method ids that appear in `methods` above OR in known list. If you cannot link BOTH endpoints to a method, DO NOT emit that edge (an edge with one dangling endpoint is useless — skip it).
4. law_constraints: `constrains` (which method this law/constraint applies to) and `consumer` (which method uses/applies this law) MUST BOTH be method ids from methods/known. A law with no linked method is USELESS — only emit a law_constraint if you can identify BOTH the method it constrains AND (if stated) the method that consumes it. If `consumer` is not stated in the passage, set consumer = constrains (the method itself uses its own law) rather than null.
5. composition_edges: `whole` and `component` must both be method ids. Emit ALL composition relations explicitly stated (X consists of Y, X = f(Y), X is a holistic model built on Y).
6. Be exhaustive within the passage: capture every distinct method, every evolution relation, every composition, every law constraint the passage states. A review window typically yields 5-15 methods and 3-10 relations.
